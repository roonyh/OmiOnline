...
class CardPlayer(webapp2.RequestHandler): 
    def post(self):
        ...
        msg = ""
        if len(table)==4:
            winner = pick_winner(table,game.trump,game.handwinner)
            ...
            game.rounds = r
            game.put()
            msg1,msg2,msg3,msg4 = create_played_msg(card,4,int(pid),winner)
        else:
            msg1,msg2,msg3,msg4 = create_played_msg(card,len(table),int(pid),(int(pid)+1)%4)  
        channel.send_message(game.user1.user_id() + game.user1.user_id(),'%s' % msg1)
        channel.send_message(game.user2.user_id() + game.user1.user_id(),'%s' % msg2)
        channel.send_message(game.user3.user_id() + game.user1.user_id(),'%s' % msg3)
        channel.send_message(game.user4.user_id() + game.user1.user_id(),'%s' % msg4)        
...
def pick_winner(table,tru,handwinner):
    tot = 0
    wincard = 0
    i = -1
    first = table[0]
...
def create_played_msg(card,num,now,next):
    ...
    message[0] = {
        ...
        'now':now,
        'next' : next,
        'mes' : 'Player%d won the last hand' % (next+1)
    }
    message[1] = {
        'type' : 'playedcard',
        'card': card,
    ...
    message[3] = {
        ...
        'now':now,
        'next' : next,
        'mes' : 'Player%d won the hand' % (next+1)
    }
    m = message[next]
    m['mes'] = "You won the hand :) Click on a card to start next hand"
